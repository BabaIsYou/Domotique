...
syno_user="xxxxxxxx"
syno_pwd="xxxxxxx"
syno_url="192.168.2.5:5000" # eg 192.168.1.100:5000
...
if [[ $(echo "$curlResult" | jq -r '.["success"]') == 'false' ]]; then 
	echo "Error on login to Surveillance Station to ${1} camera ${2}"
		exit 0
		fi
...
if [[ $(echo "$curlResult" | jq -r '.["success"]') == 'false' ]]; then 
							curl -s "http://${syno_url}/webapi/${authPath}?api=SYNO.API.Auth&method=Logout&version=${vAuth}&_sid=${SID}" > /dev/null 2>&1
									echo "Error on enabling camera ${2}"
											exit 0
												fi
												...
												# Disable action
													...
													# Enable trough DSM API
														curlResult=$(curl -s "http://${syno_url}/webapi/${surveillancePath}?api=SYNO.SurveillanceStation.Camera&method=Disable&version=${vCam}&cameraIds=${2}&_sid=${SID}")
															if [[ $(echo "$curlResult" | jq -r '.["success"]') == 'false' ]]; then 
																	curl -s "http://${syno_url}/webapi/${authPath}?api=SYNO.API.Auth&method=Logout&version=${vAuth}&_sid=${SID}" > /dev/null 2>&1
																			echo "Error on disabling camera ${2}"
																					exit 0
																						fi
# Check action
		elif [[ $1 == 'check' ]]; then
...
#echo "$curlResult"
#echo $curlResult |jq -r '.data.cameras[].enabled'
